...
from utils import vis_segmentation  #vis_segmentation，这是可视化函数，把输入图像和预测分割结果组合显示
with open("config/config.yaml", "r",encoding="utf-8") as f:
    config = yaml.safe_load(f)

image = Image.open(op.join(config["DATA_PATH"], "train/iid000757/rgb.jpg"))
mask = Image.open(op.join(config["DATA_PATH"], "train/iid000757/labels.png"))
