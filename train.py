...
parameter_count = sum(p.numel() for p in model.model.parameters()) #统计模型总参数量，用于轻量化分析
trainable_parameter_count = sum(p.numel() for p in model.model.parameters() if p.requires_grad) #统计可训练参数量

run_name = f"deeplabv3_{config['BACKBONE']}_{config['NUM_EPOCHS']}epoch"
save_model_path = config.get("SAVE_MODEL_PATH") or f"runs/{config['BACKBONE']}_v1.{config['NUM_EPOCHS']}.pt"
#初始化wandb
run = wandb.init(
    entity="a3071093490-k", #表示 wandb 账户或团队名。
    ...
    config={  #本次实验的关键信息
        ...
        "num_mask_channels": config.get("NUM_MASK_CHANNELS"),
        "crop_size": config.get("CROP_SIZE"),
        "class_weights": config.get("CLASS_WEIGHTS"),
        "optimizer": "AdamW",
        "data_path": config.get("DATA_PATH"),
        "train_samples": len(dataloaders["train"].dataset),
        "valid_samples": len(dataloaders["valid"].dataset),
